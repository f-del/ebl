...
import { shallow, configure, mount, unmount, render } from "enzyme";
...
import {
  ...
  startCard,
  createCardSuccess
} from "../src/redux/modules/cards";
import reducer from "../src/redux/store/index";
import CreateCard from "../src/components/CreateCard";
import cardReducer from "../src/redux/modules/cards";
...
describe("Unit tests", () => {
  ...
  describe("Componant tests", () => {
    ...
    test("Click on create new task and validate by ENTER", () => {
      ...
      const validateCreate = jest.fn();
      const preventDefault = jest.fn();
      wrapper.setProps({ validateCreate });
      ...
      expect(validateCreate.mock.calls.length).toBe(1);
      expect(validateCreate.mock.calls[0][0]).toBe(text);
      expectInitState(wrapper);
